...
interface ParallaxComponentV2Props {
  heading?: string;
  subheading?: string;
}

export function ParallaxComponentV2({
  heading = "Nestor Walters",
  subheading = "Veteran  ·  Writer  ·  Earth Scientist",
}: ParallaxComponentV2Props = {}) {
    ...
    <div className="relative w-full overflow-hidden" ref={parallaxRef} style={{ backgroundColor: "oklch(0.08 0.005 55)" }}>
      <section className="relative z-2 flex min-h-svh items-center justify-center py-[10em] px-4">
        <div className="absolute top-0 left-0 h-[120%] w-full">
          ...
          {/* Bottom vignette — unified warm black */}
          <div
            className="absolute bottom-0 left-0 z-30 h-[20%] w-full"
            style={{
              background: "linear-gradient(to bottom, transparent, oklch(0.08 0.005 55))",
            }}
          />
        </div>
      </section>
